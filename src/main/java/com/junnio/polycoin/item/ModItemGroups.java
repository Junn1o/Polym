...
public class ModItemGroups {
    ...
    public static ItemGroup registerItemGroup(String name, Function<RegistryKey<ItemGroup>, ItemGroup> groupFactory) {
        RegistryKey<ItemGroup> groupKey = RegistryKey.of(RegistryKeys.ITEM_GROUP, Identifier.of(PoLymCoin.MOD_ID, name));
        ItemGroup itemGroup = groupFactory.apply(groupKey);
        Registry.register(Registries.ITEM_GROUP, groupKey, itemGroup);
        return itemGroup;
    }
    public static void initialize(){
        ItemGroupEvents.modifyEntriesEvent(RegistryKey.of(RegistryKeys.ITEM_GROUP, Identifier.of(PoLymCoin.MOD_ID, "item_group")))
                .register(entries -> {
